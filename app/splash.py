...
Win32 through ctypes rather than tkinter: one small window is not worth the
whole Tcl/Tk runtime in the onefile exe (ADR-0012 counts what it carries).
Everything here is best-effort -- if the window cannot be created it reports
that through `on_warning` and the App starts exactly as it did before.
"""
import ctypes
import queue
import sys
